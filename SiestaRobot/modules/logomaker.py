...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@EnmuXLogo", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Enmu Kizuki](https://t.me/Enmu_kizuki_bot)",
...
async def logo_(event):
    ...
    if event.reply_to_msg_id:
        ...
        pics = []
        async for i in ubot2.iter_messages(
            "@EnmuLogoXGirly", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Enmu](https://t.me/Enmu_kizuki_bot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
